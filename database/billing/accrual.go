...
const accrualCoordinationTTL = 2 * time.Second

// InvalidateAccrualCache forces the next billing read to re-check newly
...
func EnsureAccruedThrough(ctx context.Context, db *gorm.DB, through time.Time) error {
	...
	type accruedVersion struct {
		PriceVersionID uint64
		Day            string
	}
	var accrued []accruedVersion
	if err := db.WithContext(ctx).Model(&models.BillingEntry{}).
		Select("price_version_id, MAX(day) AS day").
		Where("type = ? AND price_version_id IS NOT NULL", EntryTypeBaseAccrual).
		Group("price_version_id").Find(&accrued).Error; err != nil {
		return fmt.Errorf("list billing accrual watermarks: %w", err)
	}
	lastDayByVersion := make(map[uint64]time.Time, len(accrued))
	for _, item := range accrued {
		day, err := time.ParseInLocation(time.DateOnly, item.Day, BeijingLocation)
		if err == nil {
			lastDayByVersion[item.PriceVersionID] = day
		}
	}
	for _, version := range versions {
		firstDay := BeijingDay(version.EffectiveFrom)
		if lastDay, ok := lastDayByVersion[version.ID]; ok && !lastDay.Before(firstDay) {
