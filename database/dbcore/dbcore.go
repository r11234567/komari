...
func buildSQLiteDSN(databaseFile string) string {
	...
	params := "_busy_timeout=1000&_txlock=immediate&_journal_mode=WAL&_synchronous=NORMAL"
	separator := "?"
...
func GetDBInstance() *gorm.DB {
	once.Do(func() {
		...
		case flags.DatabaseTypeSQLite:
			...
			// 都生效：
			//   - _busy_timeout=1000：短暂等待后交给应用层有界退避重试，避免
			...
			if sqlDB, dbErr := instance.DB(); dbErr == nil {
				// Keep every legacy write path on one connection. History and export
				// queries use the dedicated WAL reader initialized below.
			...
			instance.Exec("PRAGMA busy_timeout = 1000;")
			instance.Exec("PRAGMA cache_size = -65536;")
			instance.Exec("PRAGMA temp_store = FILE;")
		...
		if flags.IsSQLite() {
			...
			log.Printf("Ensuring Ping task/time history index; the first startup may take a while on a large database")
			if err := instance.Exec("CREATE INDEX IF NOT EXISTS idx_ping_record_task_time ON ping_records(task_id, time)").Error; err != nil {
				log.Printf("Failed to create Ping task/time index: %v", err)
			} else {
				log.Printf("Ping task/time history index is ready")
			}
			if err := instance.Exec("PRAGMA optimize;").Error; err != nil {
				log.Printf("Failed to optimize SQLite query planner statistics: %v", err)
			}

			readInstance, err = gorm.Open(sqlite.Open(buildSQLiteDSN(flags.DatabaseFile)), logConfig)
			if err != nil {
				log.Printf("Failed to open dedicated SQLite history reader: %v", err)
				readInstance = instance
			} else {
				if readSQL, dbErr := readInstance.DB(); dbErr == nil {
					readSQL.SetMaxOpenConns(1)
					readSQL.SetMaxIdleConns(1)
					readSQL.SetConnMaxLifetime(0)
				}
				readInstance.Exec("PRAGMA query_only = ON;")
				readInstance.Exec("PRAGMA cache_size = -16384;")
				readInstance.Exec("PRAGMA temp_store = FILE;")
			}
		}
...
// GetReadDBInstance returns the isolated WAL reader used by bounded history
// queries. Other database reads keep their existing behavior.
func GetReadDBInstance() *gorm.DB {
	db := GetDBInstance()
	if readInstance != nil {
		return readInstance
	}
