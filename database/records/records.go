...
func compactRecord(db *gorm.DB) error {
...
func migrateOldRecordsAt(db *gorm.DB, now time.Time) error {
	...
	var records []models.Record
	if err := db.Table("records").Where("time >= ? AND time < ?", windowStart, windowEnd).Find(&records).Error; err != nil {
		return err
	}
