...
func (r *Runtime) loadPathModule(vm *goja.Runtime, module *goja.Object) {
	defaultPath := r.pathExports(vm, runtime.GOOS == "windows")
	_ = defaultPath.Set("posix", r.pathExports(vm, false))
	_ = defaultPath.Set("win32", r.pathExports(vm, true))
	_ = module.Set("exports", defaultPath)
}

func (r *Runtime) pathExports(vm *goja.Runtime, windows bool) *goja.Object {
	...
	resolve := func(parts []string) string {
		if windows {
			return winPathResolve(r.nodeCwd, parts)
		}
		return posixPathResolve(r.nodeCwd, parts)
	}
	...
	_ = exports.Set("relative", func(from, to string) string {
		if windows {
			return winPathRelative(r.nodeCwd, from, to)
		}
		return posixPathRelative(r.nodeCwd, from, to)
	})
	...
	_ = exports.Set("parse", func(value string) map[string]string {
		root := ""
		if windows {
			volume, rootSeparator, _ := splitWinPath(value)
			root = volume + rootSeparator
		} else if strings.HasPrefix(value, "/") {
			root = "/"
		}
		resultDir, resultBase := dirname(value), basename(value)
		extension := extname(resultBase)
		return map[string]string{"root": root, "dir": resultDir, "base": resultBase, "ext": extension, "name": strings.TrimSuffix(resultBase, extension)}
	})
	...
	_ = exports.Set("toNamespacedPath", func(value string) string {
		...
		resolved := winPathResolve(r.nodeCwd, []string{value})
...
func winPathResolve(cwd string, parts []string) string {
	...
	for index := len(parts) - 1; index >= -1; index-- {
		...
		_, root, _ := splitWinPath(part)
		if root != "" {
			break
		}
	}
	return winPathNormalize(resolved)
}
