...
func TestAggregateRollupSkipsDigestForNonPercentile(t *testing.T) {
	...
	if _, err := s.db.ExecContext(ctx, fmt.Sprintf("UPDATE %s SET digest = ?", s.tables.rollupValues), []byte("invalid")); err != nil {
...
func TestSeriesHonorsRequestedWindowInsideLongerMetricRetention(t *testing.T) {
	...
	got, err := s.Series(ctx, AggregateQuery{
		Query:       Query{MetricName: "ping.latency_ms", EntityID: "n1", Start: now.Add(-30 * 24 * time.Hour), End: now},
		Aggregation: AggLast,
		Interval:    time.Hour,
	}, now)
	if err != nil {
	...
	full, err := s.Series(ctx, AggregateQuery{
		Query:       Query{MetricName: "ping.latency_ms", EntityID: "n1", Start: now.Add(-60 * 24 * time.Hour), End: now},
		Aggregation: AggLast,
		Interval:    time.Hour,
	}, now)
	if err != nil {
