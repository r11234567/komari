...
// RegisterPprofRoutes exposes diagnostics only below the authenticated admin group.
...
func pprofSummaryHandler(c *gin.Context) {
	...
	response := pprofSummary{Profiles: make([]pprofProfileInfo, 0, len(runtimeProfileNames)+2)}
	response.Profiles = append(response.Profiles,
		pprofProfileInfo{Name: "cpu", Endpoint: "/api/admin/pprof/profile", Timed: true},
		pprofProfileInfo{Name: "trace", Endpoint: "/api/admin/pprof/trace", Timed: true},
	)
	for _, name := range runtimeProfileNames {
		...
		response.Profiles = append(response.Profiles, pprofProfileInfo{
			Name: name, Endpoint: "/api/admin/pprof/" + name,
			Samples: profile.Count(), Preview: "/api/admin/pprof/" + name + "?format=text",
		})
	}

	response.Runtime.Goroutines = runtime.NumGoroutine()
	response.Runtime.Memory = pprofMemorySummary{
		HeapAlloc: mem.HeapAlloc, HeapInuse: mem.HeapInuse,
		HeapObjects: mem.HeapObjects, Sys: mem.Sys,
	}
...
func pprofDuration(c *gin.Context) (time.Duration, bool) {
	...
	if err != nil {
		api.RespondError(c, http.StatusBadRequest, fmt.Sprintf(
			...
			minPprofDurationSeconds, maxPprofDurationSeconds,
		))
		return 0, false
	}
