...
func installedThemes() ([]models.Theme, error) {
	...
	for _, entry := range entries {
		...
		configPath, pathErr := safepath.JoinUnder("./data/theme", filepath.Join(entry.Name(), "komari-theme.json"))
		if pathErr != nil {
			continue
		}
		theme, err := loadThemeConfig(configPath)
...
func DeleteTheme(c *gin.Context) {
	...
	if err != nil {
		api.RespondError(c, http.StatusInternalServerError, "打开主题目录失败: "+err.Error())
		return
	}
	defer themesRoot.Close()
	tombstone, err := os.MkdirTemp("./data/theme", ".deleted-")
	if err != nil {
		if currentTheme == req.Short {
	...
	_ = themesRoot.Remove(tombstoneName)
	if err := themesRoot.Rename(req.Short, tombstoneName); err != nil {
		if currentTheme == req.Short {
			_ = config.Set(config.ThemeKey, currentTheme)
		}
	...
	if err := themesRoot.RemoveAll(tombstoneName); err != nil {
...
func extractAndValidateTheme(zipPath string) (models.Theme, error) {
	...
	stageDir, err := os.MkdirTemp(themesDir, ".stage-")
	if err != nil {
	...
	if err != nil {
		return themeInfo, fmt.Errorf("打开主题暂存目录失败: %v", err)
	}
	...
	for _, f := range r.File {
		...
		if !f.FileInfo().IsDir() && !f.Mode().IsRegular() {
			return themeInfo, fmt.Errorf("主题包含不支持的非普通文件: %s", f.Name)
		}

		if f.FileInfo().IsDir() {
			if err := stageRoot.MkdirAll(cleanName, 0755); err != nil {
				return themeInfo, fmt.Errorf("创建目录失败: %v", err)
			}
			continue
		}

		if parent := filepath.Dir(cleanName); parent != "." {
			if err := stageRoot.MkdirAll(parent, 0755); err != nil {
				return themeInfo, fmt.Errorf("创建目录失败: %v", err)
			}
		}
		...
		outFile, err := stageRoot.OpenFile(cleanName, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
		if err != nil {
			rc.Close()
	...
	themeDir, err := safepath.JoinUnder(themesDir, themeInfo.Short)
	if err != nil {
		return themeInfo, err
	}
	backupDir, err := os.MkdirTemp(themesDir, ".previous-")
	if err != nil {
...
func loadThemeConfig(configPath string) (models.Theme, error) {
	...
	defer root.Close()
	file, err := root.Open(filepath.Base(configPath))
	if err != nil {
		return themeInfo, err
	}
	defer file.Close()
...
func isPrivateIP(host string) bool {
	...
	for _, address := range ips {
		if blockedThemeAddress(address.IP) {
			return true
		}
	}
	return false
...
// DNS rebinding and redirect races left by URL-only validation.
func dialPublicAddress(ctx context.Context, network, address string) (net.Conn, error) {
	...
	for _, candidate := range addresses {
		if blockedThemeAddress(candidate.IP) {
			continue
		}
	...
	return nil, errors.New("destination has no permitted public address")
}
...
func downloadThemeFromURL(rawURL string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("创建主题下载请求失败: %v", err)
	}
	// lgtm[go/request-forgery] The custom transport validates the actual dial IP
	// for the initial request and every redirect, preventing DNS rebinding.
	resp, err := themeHTTPClient.Do(request)
	if err != nil {
...
func UpdateTheme(c *gin.Context) {
	...
	// 检查主题是否存在
	themeDir, err := safepath.JoinUnder("./data/theme", req.Short)
	if err != nil {
		api.RespondError(c, http.StatusBadRequest, "无效的主题路径")
		return
	}
	themeConfigPath, err := safepath.JoinUnder(themeDir, "komari-theme.json")
	if err != nil {
		api.RespondError(c, http.StatusBadRequest, "无效的主题配置路径")
		return
	}
	...
	defer themesRoot.Close()
	if _, err := themesRoot.Stat(filepath.Join(req.Short, "komari-theme.json")); os.IsNotExist(err) {
		api.RespondError(c, http.StatusNotFound, "主题不存在")
		return
	}
...
func ImportTheme(c *gin.Context) {
	...
	if preview == "true" {
		...
		// 检查是否已存在同名主题
		exists := false
		themeDir, pathErr := safepath.JoinUnder("./data/theme", themeInfo.Short)
		if pathErr != nil {
			api.RespondError(c, http.StatusBadRequest, "无效的主题路径")
			return
		}
