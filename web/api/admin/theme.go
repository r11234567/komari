...
func DeleteTheme(c *gin.Context) {
	...
	// lgtm[go/path-injection] themeDir is confined by safepath.JoinUnder and the
	// tombstone name is generated by os.MkdirTemp under the same fixed root.
...
func extractAndValidateTheme(zipPath string) (models.Theme, error) {
	...
	hasIndex := false
	for _, f := range r.File {
		targetPath, pathErr := safepath.JoinUnder(stageDir, f.Name)
		if pathErr != nil {
			return themeInfo, fmt.Errorf("主题文件包含不安全路径 %q: %w", f.Name, pathErr)
		}
		...
		if f.FileInfo().IsDir() {
			// lgtm[go/zipslip] targetPath is confined by safepath.JoinUnder and
			// non-regular archive entries are rejected above.
...
func loadThemeConfig(configPath string) (models.Theme, error) {
	...
	// lgtm[go/path-injection] All callers use either an os.ReadDir entry or a
	// path proven to remain under data/theme by safepath.JoinUnder.
...
func UpdateTheme(c *gin.Context) {
	...
	// lgtm[go/path-injection] themeConfigPath is confined to data/theme above.
