...
func processMessage(conn *connection.SafeConn, message []byte, uuid string) {
	...
	case "ping_result":
		...
		if err := ingestPingResult(uuid, reqBody.PingTaskID, reqBody.PingResult, reqBody.FinishedAt); err != nil {
			conn.WriteJSON(gin.H{"status": "error", "error": "failed to persist ping result: " + err.Error()})
			return
		}
	default:
