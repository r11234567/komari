...
func getVerifiedAuth(ctx context.Context) (string, error) {
	uuid, secret, err := getAuth(ctx)
	if err != nil {
		return "", err
	}
	authenticatedUUID, err := clients.GetClientUUIDByToken(secret)
	if err != nil || authenticatedUUID != uuid {
		return "", errors.New("unauthorized: invalid client_uuid/client_secret")
	}
	return uuid, nil
}
...
func upsertClientFromHost(uuid, secret string, h *proto.Host) error {
	if h == nil {
		return errors.New("host information is required")
	}
	...
	clientName := "nezha_" + uuid
	if len(uuid) > 8 {
		clientName = "nezha_" + uuid[:8]
	}
	c := models.Client{
		UUID:             uuid,
		Token:            secret,
...
func ingestState(uuid string, st *proto.State) error {
	...
	if err := db.Where("uuid = ?", uuid).First(&client).Error; err != nil {
		...
		client = models.Client{UUID: uuid, Token: "", Name: clientName}
	...
	rep := v1.Report{
		...
		Connections: v1.ConnectionsReport{
			TCP: int(st.TcpConnCount),
			UDP: int(st.UdpConnCount),
		},
	...
	if len(st.Gpu) > 0 {
		gpu := &v1.GPUDetailReport{Count: len(st.Gpu)}
		for index, usage := range st.Gpu {
			gpu.AverageUsage += usage
			gpu.DetailedInfo = append(gpu.DetailedInfo, v1.GPUDeviceInfo{
				Name:        fmt.Sprintf("GPU %d", index),
				Utilization: usage,
			})
		}
		gpu.AverageUsage /= float64(len(st.Gpu))
		rep.GPU = gpu
	}
	rep.UUID = uuid
	if err := clients.ReportVerify(rep); err != nil {
		return err
	}
	savedReport, err := metricstore.WriteReport(context.Background(), rep)
	if err != nil {
		return err
	}
