...
func getRecords(ctx context.Context, req *rpc.JsonRpcRequest) (any, *rpc.JsonRpcError) {
	...
	case "load":
		...
		if params.UUID != "" {
			result, err := history.Query(ctx, history.QueryRequest{
				Type: "load", UUID: params.UUID, Start: startTime.Format(time.RFC3339),
				End: endTime.Format(time.RFC3339), MaxPoints: maxCount,
			})
			if err != nil {
				return nil, historyRPCError(err, "Failed to fetch records")
			}
			recs = historyLoadRecords(result, params.UUID)
		} else {
			...
			perClient := maxCount
			if visibleCount > 0 {
				perClient = maxCount / visibleCount
				if perClient < 100 {
					perClient = 100
				}
			}
			for _, client := range clientList {
				...
				result, err := history.Query(ctx, history.QueryRequest{
					Type: "load", UUID: client.UUID, Start: startTime.Format(time.RFC3339),
					End: endTime.Format(time.RFC3339), MaxPoints: perClient,
				})
				if err != nil {
					return nil, historyRPCError(err, "Failed to fetch records")
				}
				recs = append(recs, historyLoadRecords(result, client.UUID)...)
			}
		}
	...
	case "ping":
		...
		maxPoints := params.MaxCount
		if maxPoints <= 0 {
			maxPoints = 4000
		}
		var results []*history.Response
		if params.UUID == "" && taskID == nil {
			...
			perTask := maxPoints
			if len(allTasks) > 0 {
				perTask = maxPoints / len(allTasks)
				if perTask < 100 {
					perTask = 100
				}
			}
			for _, task := range allTasks {
				id := task.Id
				result, err := history.Query(ctx, history.QueryRequest{Type: "ping", TaskID: &id, Start: startTime.Format(time.RFC3339), End: endTime.Format(time.RFC3339), MaxPoints: perTask})
				if err != nil {
					return nil, historyRPCError(err, "Failed to fetch ping records")
				}
				results = append(results, result)
			}
		} else {
			result, err := history.Query(ctx, history.QueryRequest{Type: "ping", UUID: params.UUID, TaskID: taskID, Start: startTime.Format(time.RFC3339), End: endTime.Format(time.RFC3339), MaxPoints: maxPoints})
			if err != nil {
				return nil, historyRPCError(err, "Failed to fetch ping records")
			}
			results = append(results, result)
		}
		recs := make([]models.PingRecord, 0, maxPoints)
		for _, result := range results {
			for _, series := range result.Series {
				for _, point := range series.Points {
					...
					recs = append(recs, models.PingRecord{Client: series.Client, TaskId: series.TaskID, Time: models.FromTime(point.Time), Value: value})
				}
			}
		}
		// hidden filter
